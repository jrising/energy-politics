function [VV1_1, VV1_2, strat_1, strat_2] = optimize_step_retex(VV2_1, VV2_2, N, C, D_C_min, D_C_max, P_f_min, P_f_max, F, state1, state2a, ...
...
for Pi = 1:2  % Pi stands for party, Pi=1 is the brown party and Pi=2 is the green party
  for Fi = 1:F  % indices for position of the current state in the coal price space
    ...
    s_current = S_C(state1);
    % Does the party currently in power stay in power?
    prob_win = election(voteModel, true, s_current, s_incum, s_oppos, eideal, vmArgs);
    ...
    for ii = 1:N
      ...
      if voteModel   % election period
        sums_a1 = sums_a1 + prob_win .* VV2_1((Pi-1)*F*C + (Fi2-1)*C + state2a) + ...
                  (1 - prob_win) .* VV2_1((2-Pi)*F*C + (Fi2-1)*C + state2a);
        sums_b1 = sums_b1 + prob_win .* VV2_1((Pi-1)*F*C + (Fi2-1)*C + state2b) + ...
                  (1 - prob_win) .* VV2_1((2-Pi)*F*C + (Fi2-1)*C + state2b);
        sums_a2 = sums_a2 + prob_win .* VV2_2((Pi-1)*F*C + (Fi2-1)*C + state2a) + ...
                  (1 - prob_win) .* VV2_2((2-Pi)*F*C + (Fi2-1)*C + state2a);
        sums_b2 = sums_b2 + prob_win .* VV2_2((Pi-1)*F*C + (Fi2-1)*C + state2b) + ...
                  (1 - prob_win) .* VV2_2((2-Pi)*F*C + (Fi2-1)*C + state2b);
      else
        sums_a1 = sums_a1 + VV2_1((Pi-1)*F*C + (Fi2-1)*C + state2a);
        sums_b1 = sums_b1 + VV2_1((Pi-1)*F*C + (Fi2-1)*C + state2b);
        sums_a2 = sums_a2 + VV2_2((Pi-1)*F*C + (Fi2-1)*C + state2a);
        sums_b2 = sums_b2 + VV2_2((Pi-1)*F*C + (Fi2-1)*C + state2b);
      end
    end
